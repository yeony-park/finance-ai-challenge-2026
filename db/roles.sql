...
-- 목적: 런타임(/api/search, M2+)을 RAG 조회와 offerings의 공개 응답 열로만 격리 — 공개 검색 경로 취약점이 원장 쓰기로 확대되는 것을 차단(09 §2.1).
-- offerings의 비공개 열·art_auction_records·re_trades·모든 쓰기·마이그레이션 권한은 이 역할에 부여하지 않는다. 그 권한은 CLI 전용 DATABASE_URL_DIRECT 자격증명에만 남긴다.
--
...
-- 상품 repository 공개 응답에 필요한 열만 허용한다. 테이블 단위 SELECT를 부여하지 않는다.
...
REVOKE SELECT (
  ...
  source_meta,
  created_at
) ON offerings FROM jeomjeom_rag_ro;
GRANT SELECT (
  offer_slug,
  category_id,
  provenance,
  ...
  detail,
  source_meta
) ON offerings TO jeomjeom_rag_ro;
