...
const buildingHubResponseSchema = z.object({
  response: z.object({
...
export const normalizeBuildingHubResponse = (
  ...
  const parsed = buildingHubResponseSchema.safeParse(document);
  if (!parsed.success) {
    throw new Error("건축물대장 API JSON 응답 형식을 인식할 수 없습니다.");
  }
...
const buildingHubCacheSchema = z
  .object({
...
export const parseBuildingHubCache = (
  ...
  const parsed = buildingHubCacheSchema.safeParse(raw);
  if (!parsed.success) {
...
const buildingRegisterResponseSchema = z.object({
  response: z.object({
    header: z
      .object({
...
export const normalizeBuildingRegisterResponse = (
  ...
  const parsed = buildingRegisterResponseSchema.safeParse(document);
  if (!parsed.success) {
...
const buildingRegisterCacheSchema = z.object({
  schemaVersion: z.literal(1),
...
export const parseBuildingRegisterCache = (
  ...
  const parsed = buildingRegisterCacheSchema.safeParse(raw);
  if (!parsed.success) {
    const reason = parsed.error.issues
      .slice(0, 3)
      .map((issue) => `${issue.path.join(".") || "(root)"}: ${issue.message}`)
