...
const BASIS_NOTE =
  "위 표시는 신고서 기재 취득원가를 같은 달·같은 품종·성별의 소도체 경락가 위에 올려놓은 위치이며, 적정성 판단이 아닙니다. 취득원가는 생체 송아지 매입가이고 경락가는 도축 후 도체 낙찰가라 단위가 다릅니다 — 두 값을 같은 값으로 견주지 않았습니다. 위치를 만들 수 없는 개체는 일치·원장 불일치 어느 쪽으로도 세지 않고 대조 불가로 남습니다.";
