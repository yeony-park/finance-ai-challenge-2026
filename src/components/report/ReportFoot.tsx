 ...
 * 판정 기준 전문은 /methodology가 갖고 있으므로 여기서는 요약과 이동 경로만 둔다.
 */
    ...
    <section className={s.section} aria-labelledby={FOOT_HEADING_ID}>
      <Reveal className={s.wrap}>
        ...
        <div className={s.footBody}>
          <p>
            모든 판정에는 신고서 원문의 위치와 공공 데이터 조회 응답이 근거로 붙습니다. 근거를
            붙일 수 없는 항목은 판정하지 않고 확인 불가로 남깁니다.
          </p>
          <p>
            &ldquo;원장에서 확인되지 않음&rdquo;은 조회 시점의 공개 기록에서 확인되지 않았다는
            사실만을 뜻하며, 그 원인을 단정하지 않습니다. 화면에 표시되는 발행사명·이력번호·소재지는
            익명화 처리된 상태이며, 마스킹은 서버에서 끝납니다.
          </p>
          <p>
        ...
        <div className={s.honesty}>
          ...
          <span>
            공공 데이터는 계속 갱신됩니다. 같은 항목이 다른 시점에는 다르게 확인될 수 있으므로
            리포트에는 조회 시각을 함께 적습니다.
          </span>
        </div>
