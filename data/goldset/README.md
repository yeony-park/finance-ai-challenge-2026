...
> **다음 단계(검수)**: 현재 222건 전건이 `pending`이라 위 명령은 정식 F1 대신 참고치(선라벨 대비 EM)만 낸다 — 정식 점수를 얻으려면 2번 검수에서 `review`를 채워야 하며, 규칙·LLM 두 경로가 모두 값을 낸 185건보다 규칙 단독인 `livestock_sex` 37건을 먼저 보는 편이 낫다(교차확인이 없는 쪽이라 오류가 남을 여지가 크다).
