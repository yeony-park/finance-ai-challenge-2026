...
function PlatformCard({ platform }: { platform: Platform }) {
  const current = platformRepository.getProducts(platform.id); const history = platformRepository.getHistory(platform.id); const aggregate = historicalOfferingRepository.getAggregate({ platformId: platform.id }); const simulatedReturn = history.filter((item) => resolvedTrackReturn(item.trackRecord) != null).length;
  return <Link className="platform-card" href={`/platforms/${platform.id}`}><DataModeBadge isDemo /><h2>{platform.name}</h2><p>운영 주체 : {platform.operatorName ?? "가상 운영 주체"}</p><dl><div><dt>현재 합성 상품</dt><dd>{current.length}건</dd></div><div><dt>합성 과거 이력</dt><dd>{history.length}건</dd></div><div><dt>매각 진행</dt><dd>{aggregate.byLifecycle.exit_in_progress}건</dd></div><div><dt>매각 완료</dt><dd>{aggregate.byLifecycle.sold}건</dd></div><div><dt>반환</dt><dd>{aggregate.byLifecycle.returned}건</dd></div><div><dt>표시 가능한 시뮬레이션 수익률</dt><dd>{simulatedReturn}건</dd></div><div><dt>데이터 모드</dt><dd>합성</dd></div></dl><strong>합성 이력 보기 →</strong></Link>;
}
