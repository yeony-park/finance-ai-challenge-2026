...
export async function GET(request:Request){
 ...
 const items=artistRepository.getList().filter(artist=>!needle||normalize(`${artist.nameKo} ${artist.nameEn??""}`).includes(needle)).map(artist=>{
  ...
  const historical=artistRepository.getHistoricalProducts(artist.id);
  const operating=historical.filter(item=>item.lifecycle==="operating"||item.lifecycle==="exit_in_progress");
  return {artist,counts:{current:current.length,operating:operating.length,historical:historical.length,reportedReturn:historical.filter(item=>item.trackRecord.sourceReportedReturnPct!=null).length,calculatedSettlementReturn:historical.filter(item=>item.trackRecord.calculatedSettlementReturnPct!=null).length},platforms:[...new Set([...current.map(item=>item.platform.name),...historical.map(item=>item.platform.name)])]};
 });
 return NextResponse.json({items,total:items.length,query,mode:"normalized_repository"});
}
