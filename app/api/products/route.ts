...
function values(search: URLSearchParams, key: string) {
  return search.getAll(key).flatMap((item) => item.split(",")).map((value) => value.trim()).filter(Boolean);
}
function integer(search: URLSearchParams, key: string, fallback: number) {
  const value = Number(search.get(key));
  return Number.isFinite(value) && value > 0 ? Math.floor(value) : fallback;
}
function decimal(search: URLSearchParams, key: string) {
...
export async function GET(request: Request) {
  ...
  const filter: HistoricalCatalogFilter & { scope: "current" | "historical" | "all" } = {
    scope,
    ...
    returnMin: decimal(search, "returnMin"),
    returnMax: decimal(search, "returnMax"),
  };
