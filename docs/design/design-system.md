...
- AI 요약은 `ProductAiSummary`로 요약 탭의 첫 내용에 배치하고, 테두리 없는 본문으로 표시한다.
...
| 분석 지표 | `ProductMetric` | 회색 면, R 12px, H4 제목, H3 값, 보조 설명은 본문 크기 |
...
- 검증 방법의 페이지 제목은 카테고리 목록과 같은 H2 크기·보통 굵기다. 섹션 제목은 H3,
  항목 제목은 H4·굵게, 설명은 `--ds-text-sm`·보통 굵기로 표시한다. HTML 제목 단계는 h1 → h2 → h3를 유지한다.
