...
| 축 C (검증) | 카테고리 분석 탭 또는 `/offers` 목록 → 공모별 리포트 상세 |
...
flowchart TD
    ...
    subgraph BROWSE["탐색"]
        ...
        L1["⑨ 검증 리포트 목록 · /offers<br/>청약 예정 · 진행 중 · 종료"]
        C1 <-- "탭 전환" --> C2
    end
...
| 요소 | 3D 공시 이미지(스크롤 120px에 축소 고정), 타이틀 분할, 검색창, 예시 질문 칩 |
| 액션 | 질문 제출 → `matchScaffold(query)` · 칩 클릭 → 해당 타깃 직행 · `×` → 검색 닫기 |
| 이동 | 검색 결과 패널(같은 화면 전환), 섹션 다이얼로 5구간 점프 |

휠 스크롤은 5개 스테이지(0 → 120px → 카테고리 → 첫걸음 → 감시 → 확인 질문)로 스냅한다.
`prefers-reduced-motion: reduce`면 스냅을 걸지 않는다.
...
| `reports` | 리포트, 대조 결과, 불일치, 정정, 공시가 실제와 다르 | 판정·근거 안내 | `/offers` |
| `none` | 그 외 | 안내 못 찾음 + 전체 칩 | 카테고리 4종 · `/offers` · `/methodology` |
...
커버리지 문장(2026년 공모 N건 중 M건 대조 + 종료 공모 사후 검증) +
탭 3개(청약 예정 · 진행 중 · 종료). 카드마다 판정 요약 · 마지막 대조 시각 · 관심 등록 · 리포트 열기.
