...
## 작업 8 — 축산 리포트 공통 셸·질병 지도 지연 로딩 (2026-09-01, 커밋 미생성)

**결정과 근거** — 한우·한돈 검증 화면이 같은 `ReportSectionFrame`·`DiseaseMapFrame`을 사용하도록 공통 외곽을 분리했다. 카테고리 고유 안내와 출처는 `additionalContent` 슬롯으로 주입한다. 질병 지도는 축종별 공통 API(`/api/livestock-disease-map?species=cattle|pig`)와 고정 높이 로딩 프레임을 두고, 영역이 뷰포트 240px 안으로 들어올 때 클라이언트 지도 번들·데이터를 로드한다. 상세 결정은 [ADR-0004](../decisions/ADR-0004-shared-livestock-report-map-template.md)에 기록했다.

**트레이드오프** — 현재 해시 기반 리포트 탭을 유지해 URL·정적 생성 계약의 대규모 변경은 피했지만, 지도 외 서버 섹션 데이터는 최초 RSC payload에 남는다. 한돈은 아직 `/offers/pig-*` 리포트가 없어 기존 긴 상세 화면 안에서 공통 지도 프레임만 소비한다. Kakao 키가 없으면 동일 프레임 안의 SVG 지도로 대체한다.

**검증 영향** — 축종별 공개 데이터 빌더와 API의 허용·거절·캐시 계약 테스트, 지도 필터·연도 집계 테스트, 임의 카테고리 탭 패널 렌더 테스트를 추가했다. 브라우저에서 한우는 질병 탭 선택 전 지도 미마운트/선택 후 로드, 한돈은 질병 영역 접근 전 미마운트/스크롤 후 로드를 확인하며 데스크톱·390px 화면의 가로 넘침도 확인한다. 전체 lint·TypeScript·테스트·production build를 게이트로 둔다.

**알려진 한계** — 지도 외 탭의 진정한 서버 데이터 지연 로딩은 `offers/[id]/[[...section]]` 같은 라우트 세그먼트 전환이 필요해 후속으로 남겼다. 한돈 공고가 공통 검증 리포트에 편입되기 전까지 한돈 지도는 카테고리 상세 페이지에 노출된다. 최신 전국 지도와 공고 당시 사건을 서로 다른 시각 레이어로 동시에 표현하는 기능은 아직 없다.
