...
# 4단계 — 공식 작품 이미지 4건 임시 예외 (2026-08-30)

## 결정과 근거

- 과거 `e4a4bf5`에 있던 공식 상품 이미지 중 현재 상품에 정확히 연결되는 4건을 사용자 승인에 따라
  임시 사용한다. 상품별 이미지·상품 원문 쌍은 `ART_PRODUCT_MEDIA_BY_ID` 한 곳에서 관리한다.
- 상품 5는 과거 구현에도 이미지가 없었으므로 다른 동명 작품을 붙이지 않고 `missing`·`null`로 둔다.
- 기존 상품 선택 버튼, `?product=` 주소 상태, Evidence Copilot 동기화는 유지한다. 이미지 원문 열기는
  버튼 안에 중첩하지 않고 별도 링크로 분리한다.
- 정책 변경의 승인 범위와 철회 조건은 `docs/decisions/ADR-0001-temporary-art-image-exception.md`에 기록한다.

## 트레이드오프

- 실제 작품 구분은 가능해지지만 외부 CDN의 가용성과 권리 재검토에 의존한다.
- 사용자의 “일단 사용” 승인은 임시 렌더 승인으로만 해석하며 권리 확보 사실로 기록하지 않는다.
- 전체 CDN 경로를 열지 않고 정확한 네 파일만 허용하므로, 이미지가 교체되면 자동 추종하지 않고 실패
  표지로 내려간다.

## 검증 영향

- 미디어 스키마·repository·상품 API에 정확한 URL allowlist와 이미지·원문 쌍 부정 테스트를 추가한다.
- Next Image optimizer는 네 URL만 허용하고 리다이렉트를 0회로 제한한다.
- 브라우저에서 네 이미지 로드, 상품 5 미등록 표지, 5/3/2열 크기, 선택 URL·Copilot 동기화를 확인한다.

## 알려진 한계

- 네 이미지의 저작권·플랫폼 이용허락 확보 여부는 이 결정에서 확인하지 않았다.
- 외부 배포 또는 심사 제출 전 권리 검토나 허가된 로컬 자산 교체가 필요하다.
