...
# 3단계 — 현석 신규 커밋 검토·공시 문서 계보 반영 (2026-08-23)

> 원천: `origin/hyunsuk` 신규 3커밋 — a0735ce(OpenDART 공식 검증 연결) · db51b18(부동산
> SOU 데이터 제거) · 24f417e(grounded OpenDART AI 리뷰 플로우 + `data/art/dart-filing-manifest.json`).

## 결정과 근거

- 이식 대상은 `dart-filing-manifest.json`의 **공시 문서 계보 사실**로 한정했다. manifest 자체가
  `lineageReviewStatus: unreviewed`·`allowAutomaticPublication: false`로 자동 발행을 금지하므로,
  반영 전 DART 원문을 접수번호 단위로 직접 열어 검토를 완료했다:
  - `20240125000013` 증권발행실적보고서(2024-01-25) — 최종 배정 총액 1,182,000,000원이 상품 1
    공모금액과 완전 일치 → 상품 1에 추가.
  - `20240403003155` 증권발행실적보고서(2024-04-03) — 발행총액 1,028,000,000원, 기초자산
    취득자금 934,951,942원 + 부대비용 93,048,058원이 상품 2의 구성 검산과 완전 일치 → 상품 2에 추가.
  - `20260513000002` **투자설명서**(2026-05-13) — 9회차 1호·2호 합산 발행 문서로 두 상품(공모
    685,000,000원 / 225,000,000원)을 모두 커버 → 상품 3에 추가, 상품 4의 종전 "증권신고서" 라벨 교정.
  - `20260512000391` [기재정정] 증권신고서와 `20260529000528` [기재정정] 증권발행실적보고서도
    원문에서 9-1·9-2 두 회차를 모두 다룸을 확인 → 상품 4에 두 건 추가(상품 4 계보: 정정신고서 →
    투자설명서 → 발행실적).
- 문서 종류 **라벨 오기 3건 교정**: `20240116000005`·`20240325000139`·`20260513000002`는 DART
  원문 표제가 전부 투자설명서인데 종전 콘텐츠는 "DART 증권신고서"로 표기하고 있었다(정규화본
  유래). `ART_FACT_LEAD`의 문서 종류 열거도 실제 링크 구성(증권신고서·투자설명서·발행실적보고서)에
  맞춰 갱신했다.
- 상품 5는 manifest에서도 "접수번호 오류·원문 미확인"으로 재확인돼 격하 문구를 그대로 유지했다.

## 트레이드오프

- **grounded AI 리뷰 플로우(24f417e) 미이식**: 요청 시 OpenAI 호출 구조라 "사용자 행동으로 LLM
  비용이 발생하지 않는다"는 현 프로덕션 성질과 챗 공개 게이트 4종(다턴 레드팀·전역 한도·킬스위치·
  강등 리허설) 선행 조건에 걸린다. 검증 스키마·문서 청크 설계는 챗 라우트 착수 시 참고 자산으로만 둔다.
- **OpenDART 라이브 수신 확인(a0735ce) 미이식**: 렌더 경로에서 외부 API를 부르지 않는 "화면은
  캐시만 읽는다" 원칙과 충돌. 대신 이번 검토처럼 사전 원문 대조를 거친 정적 사실만 싣는다.
- 발행실적 최초본(9회차, 2026-05-20 접수)의 접수번호는 확보하지 못해 합산발행 정정본
  (`20260529000528`)만 링크한다. 정정본이 최신 확정치라 정보 손실은 없다.

## 검증 영향

- DART 원문 대조: 접수번호 7건 전건 표제 확인 + 발행실적 2건 금액 대사 + 9회차 3건 회차 커버리지
  본문 확인(위 근거 절).
- 스위트 1345 passed / 2 skipped · tsc 0 · eslint 0 · build 21페이지 green(/art static 유지).
- `next start` + Playwright 1440 육안: 상품 1~4 카드에 문서 계보(투자설명서·발행실적 등) 렌더,
  상품 5 격하 문구 유지, 접이식 상세의 접수번호 좌표 정상.

## 알려진 한계

- 문서 계보는 접수번호·문서 종류·기준일까지만. 제출→정정 승계 관계(supersedes)의 시각 타임라인은
  가축 카테고리와 같은 watch 캐시 연결이 없어 이번 범위에서 만들지 않았다.
- manifest의 `declaredRole`은 원문 대조 결과와 1건 불일치했다(20260513000002를 "final 신고서"로
  선언했으나 실제는 투자설명서). 현석 브랜치 쪽 교정은 담당(현석) 몫으로 남긴다.
