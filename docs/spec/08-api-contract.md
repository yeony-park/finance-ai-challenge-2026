...
- API 라우트는 다음 4가지 예외에만 존재한다:
  ...
  5. **AI 상품 문답 (한시 표면·일몰 조항)** — `POST /api/ai/ask-product`. 미술 상품 문맥 한정
     Q&A(라이브 LLM)로, "대화 표면은 `/api/search` 단일" 결정(하단 팀 결정 표)의 **한시적 예외**다.
     존속 요건: ①내구(KV) 레이트리미터 ②출력 필터·요청 가드·입력 스크리닝 경유 ③상품 문맥 밖
     자유 대화로 확장 금지. **일몰 트리거**: `/api/search`가 06 4게이트를 통과해 개통되는 시점에
     본 표면을 통합·폐지한다. (2026-08-31 기록 — 구현이 계약보다 먼저 나온 것을 소급 명세화)
