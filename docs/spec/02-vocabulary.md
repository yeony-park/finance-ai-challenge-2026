...
## 카테고리 라벨 = 검증 가능 범위 (2026-08-16 오너 확정)

카테고리의 표면 라벨은 **검증 경로가 실제로 닿는 범위**를 말해야 한다 — 원장이 조회하지 못하는 대상까지 포괄하는 라벨은 검증 범위의 과대 표기다. 카테고리명은 브랜딩이기 전에 유입 질의의 분포를 통제하는 첫 번째 필터다.

- 축산 계열의 원장(축산물이력제)은 국내산만 조회 가능하므로 라벨은 `한우`·`한돈`이다 — "소"·"돼지"는 수입산 질의를 유도해 "대조 불가"를 양산한다. 코드 id(`cattle`/`pig`)는 불변.
- "한돈"의 낮은 인지도는 첫 노출 병기 **"한돈(국산 돼지)"**로 해소한다. 검색·질의 별칭(돼지·양돈)은 매처에 유지한다 — 라벨이 바뀌어도 유입 어휘는 수용한다.
- 미술품·부동산 라벨은 담당 구현의 층별 지원 선언 확정 시 같은 기준으로 감사한다 `[담당 확인 대기]`.
- 공시 원문 인용부는 개명 대상이 아니다 — 원문에 등장하는 "돼지고기"·"아프리카돼지열병" 등은 충실 인용을 유지한다.
