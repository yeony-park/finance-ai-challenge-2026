...
## Acceptance — 전건 충족 (2026-08-15 재확인, 테스트 523/523)

- [x] `npm run verify` 1회로 뱅카우 9호: 신고서 입력→claim 추출→3층위 대조→판정→근거 리포트 완주
- [x] 스냅샷 실측(36/37·학산 24호)과 동일 판정 재현 (회귀 고정)
- [x] 판정 근거 부착률 100% — 근거 0건 판정이 타입·테스트로 차단됨
- [x] fake 모드 완주 (키·네트워크 불필요) — 팀원·CI 동작
- [x] 기존 spine 테스트·레드팀 무회귀 (레드팀 24/24, S3 도메인 교체 후 재실행)
